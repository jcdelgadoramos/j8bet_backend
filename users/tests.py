...
class QueryTest(TestCase):
    ...
    def test_01_get_all_users(self):
        ...
        self.assertEqual(
            self.basic_user.username, result.data["users"][0]["name"]
        )
    ...
    def test_02_get_single_user(self):
        ...
        self.assertEqual(
            self.basic_user.id, result.data["user"]["id"]
        )
        self.assertEqual(
            self.basic_user.username, result.data["user"]["name"]
        )
        query_by_username = """
        ...
        """.format(id=self.basic_user.username)
        result = self.schema.execute(query_by_id)
        self.assertEqual(
            self.basic_user.id, result.data["user"]["id"]
        )
...
#        )
#        self.assertNotNull(token_response["data"]["tokenAuth"]["token"])
#